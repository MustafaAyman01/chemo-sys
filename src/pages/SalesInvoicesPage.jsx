...
import { Plus, Eye, Loader2, FileText, Printer, DollarSign } from 'lucide-react'
import { useForm, useFieldArray } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { z } from 'zod'
import { useSalesInvoices, useCreateSalesInvoice, useUpdateSalesInvoicePayment } from '../hooks/useSales'
...
function InvoiceForm({ onSubmit }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} id="si-form">
    ...
    <div>
      <div style={{ background: '#f8f9fb', borderRadius: 8, padding: 14, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
          ...
          <span style={{ fontFamily: 'Inter', fontWeight: 700 }}>{(+invoice?.total_amount).toLocaleString()} EGP</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
          <span style={{ color: '#6b7280' }}>المتبقي</span>
          <span style={{ fontFamily: 'Inter', fontWeight: 700, color: '#dc2626' }}>{(+invoice?.remaining_amount).toLocaleString()} EGP</span>
        </div>
      </div>

      <FormField label="مبلغ الدفعة" required>
        <input type="number" step="0.01" className="form-input ltr" value={amount} onChange={e => setAmount(e.target.value)} dir="ltr" />
      </FormField>
      <div style={{ height: 12 }} />
    ...
    <div style={{ padding: 24, fontFamily: 'Cairo' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, paddingBottom: 20, borderBottom: '2px solid #1a1d23' }}>
        ...
        <div style={{ textAlign: 'left' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#3d62f3' }}>فاتورة بيع ضريبية</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 4 }}>العميل</div>
          <div style={{ fontWeight: 700 }}>{invoice.customer?.name}</div>
          {invoice.customer?.tax_number && (
            <div style={{ fontSize: 12, color: '#6b7280' }}>الرقم الضريبي: {invoice.customer.tax_number}</div>
          )}
        </div>
        <div style={{ textAlign: 'left' }}>
        ...
        <tbody>
          {invoice.items?.map(item => (
            <tr key={item.id} style={{ borderBottom: '1px solid #e8eaed' }}>
              ...
              <td style={{ padding: 8 }} className="num">{(+item.unit_price).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div style={{ minWidth: 260 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
            <span>المجموع</span><span className="num">{(+invoice.subtotal).toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP</span>
          </div>
          {invoice.discount_amount > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
              <span>الخصم</span><span className="num">−{(+invoice.discount_amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP</span>
            </div>
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
            <span>الضريبة</span><span className="num">{(+invoice.tax_amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP</span>
          </div>
          <div style={{ borderTop: '2px solid #1a1d23', paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 16 }}>
            <span>الإجمالي</span><span className="num">{(+invoice.total_amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP</span>
          </div>
        </div>
      </div>
    </div>
...
export default function SalesInvoicesPage() {
  ...
  const { data: invoices = [], isLoading } = useSalesInvoices(filters)
  const createMutation  = useCreateSalesInvoice()
  const paymentMutation = useUpdateSalesInvoicePayment()
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className="card">
          <EmptyState icon={FileText} title="لا يوجد فواتير بيع"
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td>
                        ...
                        {inv.order?.order_number && <div style={{ fontSize: 11, color: '#9ca3af' }}>{inv.order.order_number}</div>}
                      </td>
                      ...
                      <td style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: 13 }}>
                        {(+inv.total_amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} EGP
                      </td>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button className="btn btn-ghost btn-sm btn-icon" onClick={() => setViewInvoice(inv)}>
                            <Eye size={14} />
                          </button>
                          <button className="btn btn-ghost btn-sm btn-icon" onClick={() => setPrintInvoice(inv)}>
                            <Printer size={14} />
                          </button>
                          {inv.status !== 'paid' && (
                            <button className="btn btn-ghost btn-sm btn-icon" style={{ color: '#16a34a' }} onClick={() => setPayInvoice(inv)}>
                              <DollarSign size={14} />
                            </button>
                          )}
                        </div>
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="فاتورة بيع جديدة" size="xl"
        footer={
          <>
            <button className="btn btn-secondary" onClick={() => setCreateOpen(false)}>إلغاء</button>
            <button className="btn btn-primary" form="si-form" type="submit" disabled={createMutation.isPending}>
